...
- اخبار مرتبط با ایران و منطقه را از منابع RSS می‌گیرد
- ترجمه می‌کند به فارسی (رایگان، بدون کلید API)
- به شکل عکس (بنر قرمز بالا با اسم منبع + متن سیاه) می‌سازد
- در تلگرام ارسال می‌کند
"""
...
# کشورهای بلندپایه برای نگه‌داری "آیت‌الله"
HIGH_RANKING_COUNTRIES = ["آمریکا", "فرانسه", "آلمان", "اسپانیا", "اسرائیل", 
                          "امارات", "قطر", "عربستان", "پاکستان", "ترکیه",
                          "ترامپ", "بایدن", "ماکرون", "شولتز", "نتانیاهو"]

# موضوعات خاص برای علامت مهم
IMPORTANT_TOPICS = {
...
def wrap_text(text, font, max_width, draw):
    """شکستن متن برای جای‌گیری در عکس"""
    words = text.split(" ")
    lines = []
    current = ""
    for word in words:
...
def make_image(source_name, persian_text, important_topic=None):
    """ساخت عکس استوری با سایز 1080x1920"""
    ...
    draw.text(((IMG_WIDTH - sw) / 2, (BANNER_HEIGHT - 58) / 2), source_shaped,
    ...
    draw.text(((IMG_WIDTH - fw) / 2, total_height - FOOTER_HEIGHT + 15),
              footer_text, font=footer_font, fill="#888888")

    # علامت مهم (گوشه بالا سمت راست)
    if important_topic:
        important_symbol = "⚠️"
        draw.text((IMG_WIDTH - 100, 20), important_symbol, font=important_font, fill="#C8102E")
...
def main():
    ...
    print(f"شروع اجرا در {datetime.now()} - مدت فعال: {RUN_DURATION_SECONDS} ثانیه")
