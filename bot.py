...
def dedupe_sentences(text):
    """جلوگیری از تکرار چندباره‌ی یک جمله در متن ترجمه‌شده."""
    if not text:
        return text
...
def strip_leading_headline_from_body(headline, body):
    """اگر متن خبر با همون تیتر شروع شده بود، اون تکرار رو حذف می‌کند."""
    if not body:
        return body
...
def get_headline_style(source_name, full_check_text):
    """رنگ و حاشیه‌ی تیتر بر اساس اولویت دسته‌بندی خبر."""
...
def draw_football_icon(img, x_center, y_bottom):
    ...
    r = size // 2 - 6
    cx, cy = size // 2, size // 2
    d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=(250, 250, 250, 235),
...
def make_image(source_name, headline_fa, body_fa, full_check_text):
    ...
    for line in headline_lines:
        ...
        draw.text((x, y), shaped, font=headline_font, fill=headline_fill,
                   stroke_width=3, stroke_fill=headline_stroke)
        y += 70
