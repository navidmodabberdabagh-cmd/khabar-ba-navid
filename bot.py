...
MAX_SEND_PER_RUN = 30  # در هر بار اجرا ۵ خبر مهم رو بفرسته
FEED_TIMEOUT = 5
...
def strip_leading_headline_from_body(headline, body):
    if not body:
        return body
    h_key = headline.strip().rstrip(".").lower()
    parts = [p.strip() for p in body.split(".") if p.strip()]
    while parts and parts[0].strip().lower() == h_key:
        parts.pop(0)
...
def apply_replacements(text):
    ...
    text = text.replace("امیرالمؤمنین", "امام اول شیعیان")
    text = text.replace("امیرالمومنین", "امام اول شیعیان")
    text = text.replace("حضرت علی", "امام اول شیعیان")
    text = text.replace("امام حسین", "امام سوم شیعیان")
    text = text.replace("امام زمان", "امام دوازدهم شیعیان")
    text = text.replace("امام عصر", "امام دوازدهم شیعیان")
    text = text.replace("امام حسن", "امام دوم شیعیان")
    text = text.replace("امام سجاد", "امام چهارم شیعیان")
    text = text.replace("(ع)", "").replace("(ره)", "")
    text = re.sub(r"حضرت\s*", "", text)
...
def detect_iran_conflict(text):
    t = text.lower()
    has_iran = any(w.lower() in t for w in IRAN_WORDS_FOR_CONFLICT)
    has_conflict = any(w.lower() in t for w in CONFLICT_WORDS)
    return has_iran and has_conflict

def get_headline_style(source_name, full_check_text):
    if detect_iran_conflict(full_check_text):
        return (225, 30, 30, 255), (255, 255, 255, 255)
    if detect_medical(source_name, full_check_text):
        return (235, 195, 30, 255), (0, 0, 0, 255)
    if detect_football(full_check_text):
        return (30, 45, 110, 255), (255, 255, 255, 255)
    if source_name in IRANIAN_SOURCES:
        return (20, 20, 20, 255), (255, 255, 255, 255)
    return (230, 120, 40, 255), (0, 0, 0, 255)
...
def draw_football_icon(img, x_center, y_bottom):
    ...
    r = size // 2 - 6
    cx, cy = size // 2, size // 2
    d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=(250, 250, 250, 235), outline=(20, 20, 20, 255), width=3)
    def pentagon(center, radius, rotation=0):
        pts = []
        for i in range(5):
            angle = math.radians(90 + rotation + i * 72)
            pts.append((center[0] + radius * math.cos(angle), center[1] - radius * math.sin(angle)))
        return pts
    d.polygon(pentagon((cx, cy), r * 0.38), fill=(25, 25, 25, 255))
    for i in range(5):
        angle = math.radians(90 + i * 72)
        px = cx + r * 0.62 * math.cos(angle)
        py = cy - r * 0.62 * math.sin(angle)
        d.polygon(pentagon((px, py), r * 0.22, rotation=i * 30), fill=(25, 25, 25, 255))
...
def draw_country_watermark(img, country, width, total_height):
    ...
    if info.get("special") == "iran":
        ...
        if info.get("flag"):
            d.rectangle([size * 0.3, size * 0.3, size * 0.7, size * 0.4], fill=(255, 255, 255, 60))
...
def make_image(source_name, headline_fa, body_fa, full_check_text):
    ...
    headline_fill, headline_stroke = get_headline_style(source_name, full_check_text)
    y = card_y + 70
    for line in headline_lines:
        shaped = shape_text(line)
        tw = draw.textlength(shaped, font=headline_font)
        x = card_x + (card_w - tw) / 2
        draw.text((x, y), shaped, font=headline_font, fill=headline_fill, stroke_width=3, stroke_fill=headline_stroke)
        y += 70
    y += 25
    for line in body_lines:
        ...
        x = card_x + (card_w - tw) / 2
        draw.text((x, y), shaped, font=body_font, fill=(255, 255, 255, 255), stroke_width=3, stroke_fill=(0, 0, 0, 255))
        y += 60
    source_line = shape_text(f"خبرگذاری: {source_name}")
    draw.text((card_x + inner_pad, card_y + card_h - 70), source_line, font=source_font, fill=(220, 220, 220, 255), stroke_width=2, stroke_fill=(0, 0, 0, 255))
    footer_y = card_y + card_h + 50
    shaped_brand = shape_text("خبر با نوید")
    bw = draw.textlength(shaped_brand, font=brand_font)
    draw.text(((IMG_WIDTH - bw) / 2, footer_y + 60), shaped_brand, font=brand_font, fill=(230, 120, 60, 255))
    insta_text = "Instagram : @NM_EST"
    iw = draw.textlength(insta_text, font=insta_font)
    draw.text(((IMG_WIDTH - iw) / 2, footer_y), insta_text, font=insta_font, fill=(210, 210, 210, 255))
    if detect_football(full_check_text):
...
def main():
    ...
    # اینجا رو تغییر دادم که ربات معطل نشه
